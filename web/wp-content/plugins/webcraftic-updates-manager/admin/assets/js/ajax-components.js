		...
		$('.factory-ajax-checkbox').on('change', function(ev) {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if( !response || !response.success ) {
						if( response.data.error_message ) {
							$.wbcr_factory_clearfy_212.app.showNotice('Error: [' + response.data.error_message + ']', 'danger');
						}
						return false;
					}

					/*var noticeId = $.wbcr_factory_clearfy_212.app.showNotice('Settings successfully updated', 'success');

					setTimeout(function() {
						$.wbcr_factory_clearfy_212.app.hideNotice(noticeId);
					}, 5000);*/
				},
				error: function(xhr, ajaxOptions, thrownError) {
					...
					$.wbcr_factory_clearfy_212.app.showNotice('Error: [' + thrownError + '] Status: [' + xhr.status + '] Error massage: [' + xhr.responseText + ']', 'danger');
				}
			});
		});
