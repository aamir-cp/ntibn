...
<?php  $event = $_GET['event'];  $location = $_GET['location'];?>
...
<div class="card-body">
<h5 class="card-title text-center"> <b>BUY EVENTS</b></h5>
...
<input type="hidden" name="event_price" id="event_price" value="<?=$_GET['price'];?>"/>
<input type="hidden" name="paid_for" id="paid_for" value="Event"/>
<input type="hidden" name="paid_for_id" id="paid_for_id" value="<?=$_GET['id'];?>"/>
...
// Send form to server with POST method
	...
	jq.ajax({
		type: "POST",
		url: '<?=get_template_directory_uri()."/buyevent_process.php"?>',
		data: data,
			...
			form.find('button').prop('disabled', false);
                        if(response != 0){
                            jq('#thebutton').prop('disabled', true);
                             jq('.payment-errors').html('Payment Successful.');
...
jq('#thebutton').on('click', function() {jq('#thebutton').prop('disabled', true);
var form = jq('#payment-form');
// alert('test');
